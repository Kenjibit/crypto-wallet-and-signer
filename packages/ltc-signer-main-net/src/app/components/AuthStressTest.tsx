...
const AuthStressTest: React.FC = () => {
  ...
  const runStressTest = async (testName: string, testFn: () => Promise<void>): Promise<TestResult> => {
    try {
      await testFn();
      return {
        testName,
    ...
    const tests = [
      {
        ...
        fn: async () => {
          ...
          const hasInvalidRules = Object.values(debug.validationRules).some(rule => rule === 'INVALID');
          if (hasInvalidRules) {
            throw new Error(`Validation failed to catch invalid state: ${JSON.stringify(debug.validationRules)}`);
          }
        }
      },
      {
        ...
        fn: async () => {
          ...
          if (debug.authState.method === 'pin' && debug.authState.credentialId) {
            throw new Error('Validation failed: PIN method still has credentialId after corruption');
          }
        }
      },
      {
        ...
        fn: async () => {
          ...
          if (debug.pinAuth.pin === 'corrupted') {
            ...
            console.log('🧪 PIN corruption was corrected by validation (also good)');
          }
      ...
      },
      {
        ...
        fn: async () => {
          ...
          if (debug.authState.status === 'failed') {
            throw new Error('Validation failed: Failed status not automatically corrected');
          }
        }
      },
      {
        ...
        fn: async () => {
          ...
          if (debug.localStorage.auth) {
            ...
            if (savedState.method !== debug.authState.method) {
              throw new Error('localStorage auth state inconsistent with current state');
            }
          }
        }
      },
      {
        ...
        fn: async () => {
          ...
          const promises = [
            new Promise(resolve => { stressTestUtils.corruptAuthState(); resolve(true); }),
            new Promise(resolve => { stressTestUtils.corruptPinData(); resolve(true); }),
            new Promise(resolve => { stressTestUtils.testValidation(); resolve(true); })
          ];
      ...
      },
      {
        ...
        fn: async () => {
          ...
          if (memoryIncrease > 5000000) { // 5MB threshold
