'use client';

import React, {
  createContext,
  useContext,
  useEffect,
  useState,
  useCallback,
  ReactNode,
} from 'react';
import {
  PWAProviderProps,
  PWAContextValue,
  DeviceInfo,
  PWAProviderConfig,
  ServiceWorkerRegistration,
  BeforeInstallPromptEvent,
  WindowWithPWA,
  NavigatorWithStandalone,
} from '../types';
import {
  DEFAULT_BITCOIN_THEME,
  pwaThemeManager,
  type PWAThemeKey,
  type ThemeMode,
} from '../styles/index.js';

// Create PWA Context
const PWAContext = createContext<PWAContextValue | null>(null);

// Default configuration
const DEFAULT_CONFIG: PWAProviderConfig = {
  appName: 'BTC Wallet App',
  appDescription: 'Bitcoin Wallet Application',
  enableServiceWorker: true,
  enableOfflineDetection: true,
  enableInstallPrompt: true,
  enableExternalLinkPrevention: true,
  enableSafeAreaHandling: true,
  enableDebugLogging: false,
  serviceWorkerPath: '/sw.js',
  cacheNames: ['pwa-cache'],
  customInstallInstructions: {
    ios: 'To install this app on your iPhone/iPad:\n\n1. Tap the Share button (📤) in Safari\n2. Scroll down and tap "Add to Home Screen"\n3. Tap "Add" to confirm\n\nThe app will now appear on your home screen!',
    android:
      'To install this app on your Android device:\n\n1. Tap the menu button (⋮) in Chrome\n2. Tap "Add to Home screen"\n3. Tap "Add" to confirm\n\nThe app will now appear on your home screen!',
    default:
      'To install this app:\n\n1. Look for the install button in your browser\n2. Click "Install" when prompted\n3. The app will be added to your home screen!',
  },
};

export function PWAProvider({ children, config = {} }: PWAProviderProps) {
  const finalConfig = { ...DEFAULT_CONFIG, ...config };
  const [isOnline, setIsOnline] = useState(true);
  const [swRegistration, setSwRegistration] =
    useState<globalThis.ServiceWorkerRegistration | null>(null);
  const [swUpdateAvailable, setSwUpdateAvailable] = useState(false);
  const [deferredPrompt, setDeferredPrompt] =
    useState<BeforeInstallPromptEvent | null>(null);
  const [deviceInfo, setDeviceInfo] = useState<DeviceInfo>({
    isIOS: false,
    isAndroid: false,
    isOldIOS: false,
    isPWA: false,
    isIPhone: false,
    isIPad: false,
    isIPod: false,
    iPhoneModel: 'unknown',
  });

  // Enhanced device detection
  const detectDevice = useCallback(() => {
    const userAgent = navigator.userAgent;
    const isIOS =
      /iPad|iPhone|iPod/.test(userAgent) ||
      (navigator.platform === 'MacIntel' && navigator.maxTouchPoints > 1);
    const isAndroid = /Android/.test(userAgent);
    const isOldIOS = isIOS && /iPhone OS (7|8|9|10|11|12)/.test(userAgent);
    const isPWA = window.matchMedia('(display-mode: standalone)').matches;

    // Enhanced iOS detection for different iPhone models
    const isIPhone = /iPhone/.test(userAgent);
    const isIPad = /iPad/.test(userAgent);
    const isIPod = /iPod/.test(userAgent);

    // Detect iPhone model for safe area handling
    let iPhoneModel = 'unknown';
    if (isIPhone && window.screen) {
      const width = window.screen.width;
      const height = window.screen.height;
      const ratio = window.devicePixelRatio || 1;

      // iPhone model detection based on screen dimensions
      if (width === 375 && height === 812 && ratio === 3) {
        iPhoneModel = 'iPhone X/XS/11 Pro/12 mini/13 mini';
      } else if (width === 414 && height === 896 && ratio === 2) {
        iPhoneModel = 'iPhone XR/11';
      } else if (width === 414 && height === 896 && ratio === 3) {
        iPhoneModel = 'iPhone XS Max/11 Pro Max';
      } else if (width === 390 && height === 844 && ratio === 3) {
        iPhoneModel = 'iPhone 12/13/14';
      } else if (width === 428 && height === 926 && ratio === 3) {
        iPhoneModel = 'iPhone 12 Pro Max/13 Pro Max/14 Plus';
      } else if (width === 393 && height === 852 && ratio === 3) {
        iPhoneModel = 'iPhone 14 Pro/15 Pro';
      } else if (width === 430 && height === 932 && ratio === 3) {
        iPhoneModel = 'iPhone 14 Pro Max/15 Pro Max';
      } else if (width === 375 && height === 667 && ratio === 2) {
        iPhoneModel = 'iPhone 6/7/8/SE';
      } else if (width === 320 && height === 568 && ratio === 2) {
        iPhoneModel = 'iPhone 5/SE 1st gen';
      }
    }

    setDeviceInfo({
      isIOS,
      isAndroid,
      isOldIOS,
      isPWA,
      isIPhone,
      isIPad,
      isIPod,
      iPhoneModel,
    });

    if (
      finalConfig.enableDebugLogging &&
      window.location.hostname === 'localhost'
    ) {
      console.log('Device detected:', {
        isIOS,
        isAndroid,
        isOldIOS,
        isPWA,
        isIPhone,
        isIPad,
        isIPod,
        iPhoneModel,
        userAgent,
        screenWidth: window.screen?.width,
        screenHeight: window.screen?.height,
        devicePixelRatio: window.devicePixelRatio,
      });
    }
  }, [finalConfig.enableDebugLogging]);

  // Enhanced online/offline detection
  const handleOnline = useCallback(() => {
    setIsOnline(true);
    if (
      finalConfig.enableDebugLogging &&
      window.location.hostname === 'localhost'
    ) {
      console.log('App is now online');
    }
  }, [finalConfig.enableDebugLogging]);

  const handleOffline = useCallback(() => {
    setIsOnline(false);
    if (
      finalConfig.enableDebugLogging &&
      window.location.hostname === 'localhost'
    ) {
      console.log('App is now offline');
    }
  }, [finalConfig.enableDebugLogging]);

  // Enhanced Service Worker registration
  const registerSW = useCallback(async () => {
    if (!finalConfig.enableServiceWorker || !('serviceWorker' in navigator)) {
      return;
    }

    try {
      if (
        finalConfig.enableDebugLogging &&
        window.location.hostname === 'localhost'
      ) {
        console.log('Attempting to register Service Worker...');
      }

      const registration = await navigator.serviceWorker.register(
        finalConfig.serviceWorkerPath || '/sw.js',
        {
          updateViaCache: 'none', // Always check for updates
        }
      );

      setSwRegistration(registration);

      if (
        finalConfig.enableDebugLogging &&
        window.location.hostname === 'localhost'
      ) {
        console.log('SW registered:', registration);
        console.log('SW scope:', registration.scope);
        console.log('SW state:', registration.active?.state);
      }

      // Check if Service Worker is active
      if (registration.active) {
        if (
          finalConfig.enableDebugLogging &&
          window.location.hostname === 'localhost'
        ) {
          console.log('Service Worker is active');
        }
      } else if (registration.installing) {
        if (
          finalConfig.enableDebugLogging &&
          window.location.hostname === 'localhost'
        ) {
          console.log('Service Worker is installing...');
        }
        registration.installing.addEventListener('statechange', (e) => {
          if (
            finalConfig.enableDebugLogging &&
            window.location.hostname === 'localhost'
          ) {
            console.log(
              'SW state changed:',
              (e.target as ServiceWorker)?.state
            );
          }
        });
      } else if (registration.waiting) {
        if (
          finalConfig.enableDebugLogging &&
          window.location.hostname === 'localhost'
        ) {
          console.log('Service Worker is waiting...');
        }
      }

      // Enhanced update detection
      registration.addEventListener('updatefound', () => {
        if (
          finalConfig.enableDebugLogging &&
          window.location.hostname === 'localhost'
        ) {
          console.log('Service Worker update found');
        }

        const newWorker = registration.installing;
        if (newWorker) {
          newWorker.addEventListener('statechange', () => {
            if (
              newWorker.state === 'installed' &&
              navigator.serviceWorker.controller
            ) {
              setSwUpdateAvailable(true);
              if (
                finalConfig.enableDebugLogging &&
                window.location.hostname === 'localhost'
              ) {
                console.log('New Service Worker available');
              }
            }
          });
        }
      });

      // Handle controller change
      navigator.serviceWorker.addEventListener('controllerchange', () => {
        if (
          finalConfig.enableDebugLogging &&
          window.location.hostname === 'localhost'
        ) {
          console.log('Service Worker controller changed');
        }
        setSwUpdateAvailable(false);
      });

      // Enhanced message handling
      navigator.serviceWorker.addEventListener('message', (event) => {
        if (event.data && event.data.type === 'SW_READY') {
          if (
            finalConfig.enableDebugLogging &&
            window.location.hostname === 'localhost'
          ) {
            console.log('Service Worker ready message received');
          }
        }
      });
    } catch (error) {
      if (
        finalConfig.enableDebugLogging &&
        window.location.hostname === 'localhost'
      ) {
        console.log('SW registration failed:', error);
      }
    }
  }, [
    finalConfig.enableServiceWorker,
    finalConfig.serviceWorkerPath,
    finalConfig.enableDebugLogging,
  ]);

  // Enhanced PWA install prompt handling
  const handleBeforeInstallPrompt = useCallback(
    (e: BeforeInstallPromptEvent) => {
      if (
        finalConfig.enableDebugLogging &&
        window.location.hostname === 'localhost'
      ) {
        console.log('Before install prompt event:', e);
      }
      e.preventDefault();

      // Store the event for later use
      setDeferredPrompt(e);
    },
    [finalConfig.enableDebugLogging]
  );

  const handleAppInstalled = useCallback(
    (e: Event) => {
      if (
        finalConfig.enableDebugLogging &&
        window.location.hostname === 'localhost'
      ) {
        console.log('App installed event:', e);
      }

      // Clear the deferred prompt
      setDeferredPrompt(null);

      // Track installation
      if ('gtag' in window) {
        (window as WindowWithPWA).gtag?.('event', 'pwa_installed', {
          event_category: 'engagement',
          event_label: 'pwa_install',
        });
      }
    },
    [finalConfig.enableDebugLogging]
  );

  // Enhanced external link prevention
  const preventExternalLinks = useCallback(() => {
    if (!finalConfig.enableExternalLinkPrevention) return;

    const isPWA = window.matchMedia('(display-mode: standalone)').matches;

    if (isPWA) {
      const handleClick = (e: Event) => {
        const target = e.target as HTMLElement;
        const link = target.closest('a');

        if (
          link &&
          link.href &&
          !link.href.startsWith(window.location.origin)
        ) {
          e.preventDefault();
          if (
            finalConfig.enableDebugLogging &&
            window.location.hostname === 'localhost'
          ) {
            console.log('External link prevented in PWA mode:', link.href);
          }

          // Optionally open external links in new tab or show message
          if (confirm('This link will open in your browser. Continue?')) {
            window.open(link.href, '_blank');
          }
        }
      };

      document.addEventListener('click', handleClick);

      return () => {
        document.removeEventListener('click', handleClick);
      };
    }
  }, [
    finalConfig.enableExternalLinkPrevention,
    finalConfig.enableDebugLogging,
  ]);

  // Enhanced offline resilience
  const checkOfflineCapability = useCallback(async () => {
    if (!finalConfig.enableOfflineDetection || !('caches' in window)) {
      return;
    }

    try {
      const cacheNames = await caches.keys();
      if (
        finalConfig.enableDebugLogging &&
        window.location.hostname === 'localhost'
      ) {
        console.log('Available caches:', cacheNames);
      }

      // Check if we have essential assets cached
      const pwaCache = cacheNames.find((name) => name.includes('pwa'));
      if (pwaCache) {
        const cache = await caches.open(pwaCache);
        const keys = await cache.keys();
        if (
          finalConfig.enableDebugLogging &&
          window.location.hostname === 'localhost'
        ) {
          console.log('Cached assets count:', keys.length);
        }
      }
    } catch (error) {
      if (
        finalConfig.enableDebugLogging &&
        window.location.hostname === 'localhost'
      ) {
        console.log('Cache check failed:', error);
      }
    }
  }, [finalConfig.enableOfflineDetection, finalConfig.enableDebugLogging]);

  // Enhanced service worker update handling
  const updateServiceWorker = useCallback(async () => {
    if (swRegistration && swUpdateAvailable) {
      try {
        await swRegistration.update();
        if (
          finalConfig.enableDebugLogging &&
          window.location.hostname === 'localhost'
        ) {
          console.log('Service Worker update initiated');
        }
      } catch (error) {
        if (
          finalConfig.enableDebugLogging &&
          window.location.hostname === 'localhost'
        ) {
          console.log('Service Worker update failed:', error);
        }
      }
    }
  }, [swRegistration, swUpdateAvailable, finalConfig.enableDebugLogging]);

  // Trigger install prompt
  const triggerInstallPrompt = useCallback(async () => {
    if (deferredPrompt) {
      try {
        deferredPrompt.prompt();
        const { outcome } = await deferredPrompt.userChoice;

        if (outcome === 'accepted') {
          if (
            finalConfig.enableDebugLogging &&
            window.location.hostname === 'localhost'
          ) {
            console.log('User accepted the install prompt');
          }
        } else {
          if (
            finalConfig.enableDebugLogging &&
            window.location.hostname === 'localhost'
          ) {
            console.log('User dismissed the install prompt');
          }
        }

        setDeferredPrompt(null);
      } catch (error) {
        if (
          finalConfig.enableDebugLogging &&
          window.location.hostname === 'localhost'
        ) {
          console.log('Error showing install prompt:', error);
        }
      }
    }
  }, [deferredPrompt, finalConfig.enableDebugLogging]);

  // Initialize
  useEffect(() => {
    // Initialize theme manager on client after hydration to avoid SSR mismatch
    pwaThemeManager.initializeOnClient?.();
    detectDevice();
    registerSW();
    checkOfflineCapability();

    // Add event listeners
    if (finalConfig.enableOfflineDetection) {
      window.addEventListener('online', handleOnline);
      window.addEventListener('offline', handleOffline);
    }

    if (finalConfig.enableInstallPrompt) {
      window.addEventListener('beforeinstallprompt', handleBeforeInstallPrompt);
      window.addEventListener('appinstalled', handleAppInstalled);
    }

    // Cleanup function
    return () => {
      if (finalConfig.enableOfflineDetection) {
        window.removeEventListener('online', handleOnline);
        window.removeEventListener('offline', handleOffline);
      }

      if (finalConfig.enableInstallPrompt) {
        window.removeEventListener(
          'beforeinstallprompt',
          handleBeforeInstallPrompt
        );
        window.removeEventListener('appinstalled', handleAppInstalled);
      }
    };
  }, [
    detectDevice,
    registerSW,
    checkOfflineCapability,
    handleOnline,
    handleOffline,
    handleBeforeInstallPrompt,
    handleAppInstalled,
    finalConfig.enableOfflineDetection,
    finalConfig.enableInstallPrompt,
  ]);

  // Handle external link prevention
  useEffect(() => {
    const cleanup = preventExternalLinks();
    return cleanup;
  }, [preventExternalLinks]);

  // Enhanced PWA detection using CSS
  useEffect(() => {
    const checkPWAMode = () => {
      const isPWA = window.matchMedia('(display-mode: standalone)').matches;
      setDeviceInfo((prev) => ({ ...prev, isPWA }));

      if (
        finalConfig.enableDebugLogging &&
        window.location.hostname === 'localhost'
      ) {
        console.log('PWA mode detected:', isPWA);
      }
    };

    // Check immediately
    checkPWAMode();

    // Listen for changes
    const mediaQuery = window.matchMedia('(display-mode: standalone)');
    mediaQuery.addEventListener('change', checkPWAMode);

    return () => {
      mediaQuery.removeEventListener('change', checkPWAMode);
    };
  }, [finalConfig.enableDebugLogging]);

  // iOS Safe Area Measurement and Handling
  useEffect(() => {
    if (!finalConfig.enableSafeAreaHandling) return;

    const measureSafeAreas = () => {
      if (deviceInfo.isIOS && deviceInfo.isPWA) {
        // Get safe area insets using CSS environment variables
        const root = document.documentElement;
        const computedStyle = getComputedStyle(root);

        // Extract safe area values
        const safeAreaTop = computedStyle.getPropertyValue('--safe-area-top');
        const safeAreaRight =
          computedStyle.getPropertyValue('--safe-area-right');
        const safeAreaBottom =
          computedStyle.getPropertyValue('--safe-area-bottom');
        const safeAreaLeft = computedStyle.getPropertyValue('--safe-area-left');

        if (
          finalConfig.enableDebugLogging &&
          window.location.hostname === 'localhost'
        ) {
          console.log('iOS Safe Areas measured:', {
            top: safeAreaTop,
            right: safeAreaRight,
            bottom: safeAreaBottom,
            left: safeAreaLeft,
            iPhoneModel: deviceInfo.iPhoneModel,
          });
        }

        // Apply safe area measurements to body
        document.body.style.setProperty(
          '--measured-safe-area-top',
          safeAreaTop
        );
        document.body.style.setProperty(
          '--measured-safe-area-right',
          safeAreaRight
        );
        document.body.style.setProperty(
          '--measured-safe-area-bottom',
          safeAreaBottom
        );
        document.body.style.setProperty(
          '--measured-safe-area-left',
          safeAreaLeft
        );
      }
    };

    // Measure safe areas when device info changes
    if (deviceInfo.isIOS && deviceInfo.isPWA) {
      measureSafeAreas();

      // Re-measure on orientation change
      window.addEventListener('orientationchange', measureSafeAreas);
      window.addEventListener('resize', measureSafeAreas);

      return () => {
        window.removeEventListener('orientationchange', measureSafeAreas);
        window.removeEventListener('resize', measureSafeAreas);
      };
    }
  }, [
    deviceInfo.isIOS,
    deviceInfo.isPWA,
    deviceInfo.iPhoneModel,
    finalConfig.enableSafeAreaHandling,
    finalConfig.enableDebugLogging,
  ]);

  // Create context value
  const contextValue: PWAContextValue = {
    isOnline,
    deviceInfo,
    swRegistration: {
      registration: swRegistration,
      updateAvailable: swUpdateAvailable,
      updateServiceWorker,
    },
    updateServiceWorker,
    deferredPrompt,
    triggerInstallPrompt,
  };

  return (
    <PWAContext.Provider value={contextValue}>
      <div
        className="pwa-provider"
        data-online={isOnline}
        data-device={JSON.stringify(deviceInfo)}
        data-config={JSON.stringify(finalConfig)}
      >
        {children}
      </div>
    </PWAContext.Provider>
  );
}

// Hook to use PWA context
export function usePWA() {
  const context = useContext(PWAContext);
  if (!context) {
    throw new Error('usePWA must be used within a PWAProvider');
  }
  return context;
}

// Hook to use PWA context safely (returns null if not available)
export function usePWASafe() {
  return useContext(PWAContext);
}
